...
resource "null_resource" "spoke_namespace" {
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      if ! kubectl get namespace ${self.triggers.namespace} --context ${self.triggers.context} --insecure-skip-tls-verify=true >/dev/null 2>&1; then
        echo "Creating namespace ${self.triggers.namespace} in cluster ${self.triggers.context}..."
        kubectl create namespace ${self.triggers.namespace} --context ${self.triggers.context} --insecure-skip-tls-verify=true
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      if ! kubectl wait --for=condition=available --timeout=${var.kubectl_timeout} \
        ...
        --context ${each.value} --insecure-skip-tls-verify=true 2>&1 | tee -a "$$LOG_FILE"; then
        ...
        kubectl describe deployment/${var.argocd_repo_server_name} -n ${var.spoke_namespace} --context ${each.value} --insecure-skip-tls-verify=true | tee -a "$$LOG_FILE"
        echo "\n=== POD STATUS ===" | tee -a "$$LOG_FILE"
        kubectl get pods -n ${var.spoke_namespace} -l app.kubernetes.io/name=${var.argocd_repo_server_name} --context ${each.value} --insecure-skip-tls-verify=true -o wide | tee -a "$$LOG_FILE"
        echo "\n=== POD LOGS (most recent) ===" | tee -a "$$LOG_FILE"
        kubectl logs -n ${var.spoke_namespace} -l app.kubernetes.io/name=${var.argocd_repo_server_name} --context ${each.value} --insecure-skip-tls-verify=true --tail=100 2>&1 | tee -a "$$LOG_FILE" || true
        echo "\n=== RECENT EVENTS ===" | tee -a "$$LOG_FILE"
        kubectl get events -n ${var.spoke_namespace} --context ${each.value} --insecure-skip-tls-verify=true --sort-by='.lastTimestamp' | head -50 | tee -a "$$LOG_FILE"
        exit 1
      fi
...
resource "null_resource" "spoke_k3s_redis_workaround" {
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      if kubectl get node -o wide --context ${each.value} --insecure-skip-tls-verify=true 2>&1 | tee -a "$$LOG_FILE" | grep -q "k3s"; then
        ...
        kubectl delete networkpolicy ${var.argocd_redis_network_policy_name} -n ${var.spoke_namespace} \
          --context ${each.value} --insecure-skip-tls-verify=true --ignore-not-found=true 2>&1 | tee -a "$$LOG_FILE"
        ...
        if ! kubectl rollout status deployment/${var.argocd_redis_name} -n ${var.spoke_namespace} \
          --context ${each.value} --insecure-skip-tls-verify=true --timeout=${var.namespace_delete_timeout} 2>&1 | tee -a "$$LOG_FILE"; then
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      if ! kubectl rollout status statefulset/${var.argocd_application_controller_name} \
        ...
        kubectl describe statefulset/${var.argocd_application_controller_name} -n ${var.spoke_namespace} --context ${each.value} --insecure-skip-tls-verify=true | tee -a "$$LOG_FILE"
        exit 1
      fi
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      if ! kubectl wait --for=condition=available --timeout=${var.kubectl_timeout} \
        ...
        kubectl describe deployment/argocd-agent-agent -n ${var.spoke_namespace} --context ${each.value} --insecure-skip-tls-verify=true | tee -a "$LOG_FILE"
        exit 1
      fi
      ...
      if [[ "$$READY_PODS" == *"True"* ]]; then
        ...
        if echo "$$AGENT_LOGS" | grep -qi "authentication successful\|connected to\|connection established"; then
          ...
          echo "  kubectl logs -l app.kubernetes.io/name=argocd-agent-agent -n ${var.spoke_namespace} --context ${each.value} --insecure-skip-tls-verify=true" | tee -a "$LOG_FILE"
        else
